...
func TestParseBulkString(t *testing.T) {
	t.Run("normal string that fits", func(t *testing.T) {
		buf := []byte("11\r\ntest string\r\n")
		b := &bytes.Buffer{}
		out, err := unmarshalBulkString(b, buf)
	...
	t.Run("Error on intRead", func(t *testing.T) {
		...
		msg, err := unmarshalBulkString(mockReader, buf)
		require.Error(t, err)
		require.Nil(t, msg)
	})
...
func TestParseSimpleString(t *testing.T) {
	t.Run("Reader with fewer bytes than buf size", func(t *testing.T) {
		...
		b := &bytes.Buffer{}
		msg, err := unmarshalSimpleString(b, buf)
		require.NoError(t, err)
	...
	t.Run("Error on read", func(t *testing.T) {
		...
		buf := []byte("This is a")
		mockReader.EXPECT().Read(gomock.Any()).Return(0, errors.New("EOF"))
		msg, err := unmarshalSimpleString(mockReader, buf)
		require.Error(t, err)
		require.Nil(t, msg)
	})
}
...
func TestReadSimpleString(t *testing.T) {
	t.Run("normal string", func(t *testing.T) {
		...
		out, err := readSimpleString(b, buf)
		require.NoError(t, err)
		require.Equal(t, []byte("test string"), out)
	})
}
...
func TestReadSwitch(t *testing.T) {
	t.Run("error string", func(t *testing.T) {
		buf := make([]byte, 0, 100)
		b := bytes.NewBuffer([]byte("-test string\r\n"))
		out, err := readSwitch('+', unmarshalSimpleString, b, buf)
		require.Nil(t, out)
	...
	t.Run("error on read", func(t *testing.T) {
		...
		buf := make([]byte, 0, 100)
		mockReader.EXPECT().Read(gomock.Any()).Return(0, errors.New("EOF"))
		msg, err := readSwitch('+', unmarshalSimpleString, mockReader, buf)
		require.Nil(t, msg)
		require.Error(t, err)
	})
